...
namespace Template.Database
{
	public class PostgresRepository : IRepository
	{
		...
		public async Task<bool> DeleteAsync<T>(long? id) where T : HaveId
		{
		...
		public async Task<T> GetAsync<T>(long? id) where T : HaveId
		{
			var find = await _context.Set<T>().FindAsync(id);
			return find;
		}

		public async Task<T[]> GetAllAsync<T>() where T : HaveId
		{
			return await _context.Set<T>().ToArrayAsync();
		}

		public async Task<bool> SaveAsync<T>(T entity) where T : HaveId
		{
		...
		public async Task<bool> UpdateAsync<T>(T entity) where T : HaveId
		{
